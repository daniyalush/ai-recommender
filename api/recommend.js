...
return { raw, normalized, family, familyLabel: family?.label || raw, ...degree };
}
...
if (!fs.existsSync(datasetPath)) throw new Error("Programs dataset not found at knowledge/Programs.json");
...
if (!response.ok) throw new Error(data.message || data.error || `HubSpot request failed: ${response.status}`);
return data;
}
...
setIfPresent(properties, HUBSPOT_PROP.gender, form.gender);
...
setIfPresent(properties, HUBSPOT_PROP.semester_interested, form.semester_interested || "Fall 2026");
...
properties[HUBSPOT_PROP.intended_program_single_field] = buildMajorsField(intendedProgram, parsedMajors.selected);
return properties;
}
...
{
role: "user",
content: [{ type: "input_text", text: `Student form data:\n${JSON.stringify(form, null, 2)}` }],
},
];
...
setIfPresent(properties, HUBSPOT_PROP.gender, body.gender);
...
const patchProps = { [HUBSPOT_PROP.source_new]: SOURCE_NEW_VALUE };
...
if (raw.includes("INVALID_OPTION") || raw.includes("Property values were not valid") || raw.includes("was not one of the allowed options")) {
return "One or more selected values are invalid. Please review the form and try again.";
}
if (raw.includes("Missing required field")) return "Please complete all required fields.";
if (raw.includes("Missing required file")) return "Please upload all required documents.";
if (raw.includes("No HubSpot contact found")) return "We could not find your profile. Please submit the form again.";
